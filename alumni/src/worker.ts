...
async function deleteCheckIn(request: Request, env: Env) {
	const checkIn = parseCheckIn(await request.json());
	if (!checkIn) return new Response('Invalid check in data', { status: 422 });
	await env.DB.prepare(`delete from check_ins where ticket_id = ?;`).bind(checkIn.ticketId).run();
	const response = new Response();
	response.headers.append('Access-Control-Allow-Origin', '*');
	return response;
}

async function answerPreflight(request: Request, env: Env) {
	const response = new Response();
	response.headers.append('Access-Control-Allow-Origin', '*');
	response.headers.append('Access-Control-Allow-Methods', 'POST, DELETE');
...
routes.set('/check_in@OPTIONS', answerPreflight);
